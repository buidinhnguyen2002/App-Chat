...
import {
    getMeetingRoom, getMeetingRoomAudio,
    getURLVideo, isAudioCall, isConnectChatPeople,
    isJoinRoomMeeting, isJoinRoomMeetingAudio, isJSON,
    isLeaveRoomMeeting, isLeaveRoomMeetingAudio, isLink, isMeetingEnd,
    isRejectRoomMeeting,
    isVideoCall
} from "../../util/function";
    ...
    return (
        <div>
            ...
            {video !== '' ? <div className={`message_container ${props.name === myName ? 'message_container-flexRight':'message_container-flexleft'}`}>
                ...
                {(props.name !== myName &&props.type === 1) && (<span className={"author_chat"}>{props.name}</span>)}
            </div> : videoCall != null || audioCall != null ? <div className={`message_container d-flex ${props.name === myName ? 'message_container-flexRight':'message_container-flexleft'}`} onClick={()=>handelJoinVideoCall(videoCall != null ? videoCall : audioCall)}>
                {props.name !== myName && <div className="message_avatar">
                    <img src={urlAvatar} alt=""/>
                    {/*<img src="data:image/jpeg;base64," alt=""/>*/}
                </div>}
                <div className={'flex-container d-flex video_call-wrapper'}>
                    <div className="ic_video_call">{videoCall != null ? <i className="fa-solid fa-video" ></i>:<i className="bi bi-telephone" style={{fontSize: "22px"}}></i>}</div>
                    <div className="main-content">
                        <h4>{videoCall != null ? "Nhóm chat video": "Cuộc gọi thoại nhóm"} </h4>
